...
function analyzeLevel(question: string) {
  ...
  const level3 = ['만약','다면','이라면','한다면','된다면','라면','바뀐다면','달라진다면','없어진다면','늘어난다면','줄어든다면','없다면','있다면','많아지면','적어지면','달라지면','사라진다면','비교','차이','다르게','어떻게 다를까','다를까','같을까','영향','관계','조건','어떻게 달라질까','달라질까','어떻게 변','더 많','더 적','어떤 차이','공통점','비슷한 점','다른 점','보다 더','반면','상황이 달라지면','조건이 바뀌면','늘어나면','줄어들면','높아지면','낮아지면','없애면','추가하면','수 있을까','가능할까','변할 수 있','바뀔 수 있','될 수 있','없으면','있으면','어떻게 될까'];
  if (level3.some(w => question.includes(w))) return 3;
  const level2 = ['왜','이유','까닭','원인','무엇 때문에','뜻','의미','특징','역할','구조','원리','개념','방법','과정','이란 무엇','란 무엇','은 무엇인가','는 무엇인가','성질','어떻게 해서','어떻게 생겨','어떻게 만들어','어떻게 되는','어떻게 일어','어떻게 작동','어떻게 움직','어떻게 되나요','어떻게 이루어','왜 그럴까','왜 그런가','왜 생기','왜 발생','왜 일어','무슨 이유','어떤 이유','어떤 원인'];
