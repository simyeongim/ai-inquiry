...
function analyzeLevel(question: string) {
  ...
  if (['왜','이유','까닭','원인','무엇 때문에'].some(w => question.includes(w))) return 2;
  ...
  if (['어떻게 해서','어떻게 생겨','어떻게 만들어','어떻게 되는','어떻게 일어','어떻게 작동','어떻게 움직','왜 그럴까','왜 그런가','왜 생기','왜 발생','왜 일어','무슨 이유','어떤 이유','어떤 원인'].some(w => question.includes(w))) return 2;
  return 1;
}
