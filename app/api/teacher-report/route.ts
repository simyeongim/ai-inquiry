...
절대 금지:
- 서로 다른 그룹의 질문을 합쳐 하나의 deepQuestion 만들기
- "우주의 자전과 식물의 상관관계" 처럼 관련 없는 두 주제를 연결하기
- 각 deepQuestion은 반드시 하나의 그룹 안에서만 출발할 것

잘못된 예: "태양계 자전 주기와 식물 성장의 관계는 어떻게 될까?" — earth_space 그룹과 plant 그룹을 억지로 합침
올바른 예:
  [earth_space 그룹] 원본: "지구는 왜 자전할까?" → 발전: "지구가 자전을 멈추면 낮과 밤은 어떻게 변할까?"
  [plant 그룹]       원본: "식물은 빛이 없으면 어떻게 될까?" → 발전: "빛의 양이 달라지면 식물의 성장은 어떻게 달라질까?"
